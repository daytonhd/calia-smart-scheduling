...
from app.services.conflict_detection import MVP_USER_ID, find_free_windows

# ---------------------------------------------------------------------------
# Thresholds — tweak here to adjust triage sensitivity. All values in minutes
# unless otherwise noted.
# ---------------------------------------------------------------------------
OVERLOADED_DAY_BUSY_MINUTES = 6 * 60        # >= 6h scheduled+blocked → overloaded
...
def compute_weekly_triage(
    ...
    blocked = session.exec(
        select(BlockedTime).where(
            ...
            BlockedTime.start_time < week_end_exclusive_dt,
            BlockedTime.end_time > week_start_dt,
        )
    ).all()

    event_intervals = [(e.start_time, e.end_time) for e in events]
    blocked_intervals = [(b.start_time, b.end_time) for b in blocked]
    ...
    for offset in range(DAYS_IN_WEEK):
        ...
        blocked_min = _clip_minutes(blocked_intervals, day_start_dt, day_end_dt)
        total_busy = scheduled + blocked_min
